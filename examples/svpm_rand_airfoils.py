...
if __name__ == '__main__':
    directory_path = Path('../Airfoil_DAT_Selig')

    # Use iterdir() to get all files and directories in the specified path
    all_files = directory_path.iterdir()

    # Filter only files (exclude directories)
    files_only = [file for file in all_files if file.is_file()]
    random_files = [Path('../Airfoil_DAT_Selig') / 'fx62k131.dat']
    num_points = 101
    for file in tqdm(random_files):
        try:
            xfoil_airfoil_dir = Path('../xfoil_usable')
            XB, YB = interpolator(file_path=file, num_points=num_points, degree=3).T
            xfoil_airfoil_dir.mkdir(parents=True, exist_ok=True)
            loc = str(xfoil_airfoil_dir) + '/' + str(file.name)[:-4] + '.txt'
            np.savetxt(loc, np.array([XB, YB]).T, fmt='%1.6f', delimiter=' ')
            airfoil = '2412'
            AoA = 6
            res = None
            x_foil_cl = 0
            ...
            try:
                x_foil_cl = xf.find_coefficients(airfoil=loc, alpha=AoA, NACA=False)['CL']
                res = xf.find_pressure_coefficients(airfoil=loc, alpha=AoA, NACA=False, )
                xfoil_cp = pd.DataFrame(res)
                xfoil_cp_upp = xfoil_cp[xfoil_cp['y'] >= 0]
                xfoil_cp_low = xfoil_cp[xfoil_cp['y'] < 0]
            except:
                print('XFOIL Error')
                res = xf.call(airfoil=loc, alfas=0, NACA=False, )
                xfoil_cp = pd.DataFrame(res)
                xfoil_cp_upp = xfoil_cp[xfoil_cp['y'] >= 0]
                xfoil_cp_low = xfoil_cp[xfoil_cp['y'] < 0]
            ...
            airfoil_name = file.stem
            fig.suptitle(f'Source Vortex Panel Method Results for {airfoil_name} at {AoA} degrees AoA', fontsize=16)
            # Panel Geometry
            ...
            axs[0, 1].streamplot(x, y, u, v, density=2.5, color='b')
            ...
            plt.savefig(f'Airfoil_Results/{airfoil_name}_{AoA}_degrees_AoA.png', dpi=300)
            plt.show()
            time_end = time.time()
